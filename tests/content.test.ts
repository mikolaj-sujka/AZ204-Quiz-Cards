...
describe('content model', () => {
  it('imports the complete source quiz with Microsoft source mapping', () => {
    expect(importedBacklogCount()).toBe(625);
    expect(importedVerifiedCount()).toBe(625);
    expect(importedExamQuestionCount()).toBe(516);
  ...
  it('keeps active questions verified and officially sourced', () => {
    expect(verifiedQuestions.length).toBeGreaterThan(500);
